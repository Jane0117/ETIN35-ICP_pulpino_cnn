...
source ${PULP_PATH}/vsim/vcompile/vcompile_dummy.csh  || exit 1
echo "${Blue}--> Process & sram & lu_pads libraries compilation complete! ${NC}"

echo ""
echo "${Purple}--> Compiling top_top_pnr.v ... ${NC}"
echo ""

# build the synthesized top file
vlog -quiet -work ${LIB_PATH} ${PULP_PATH}/../PNR_CNN/top_top_pnr_g5.v    || exit 1
#vlog -quiet -work ${LIB_PATH} ${PULP_PATH}/../PNR_CNN/top_top_test.v    || exit 1
#vlog -quiet -work ${LIB_PATH} ${PULP_PATH}/../PNR_2/top_top_pnr2.v    || exit 1
echo "${Blue}--> top_top_pnr.v compilation complete! ${NC}"

# build the testbench files
echo ""
echo "${Purple}--> Compiling Pulpino testbench... ${NC}"
echo ""
...
echo "${Blue}--> Pulpino testbench compilation complete! ${NC}"
